...
class Laboratory:
    ...
    def __init__(self):
         ...
         self.laboratory_cost = "None"

    def addLaboratory(self):
        '''Adds and writes the Laboratory name to the file'''
        self.open_file = open("laboratories.txt", "r")
        self.laboratory_list = self.open_file.readlines()
        self.open_file.close()

        self.laboratory_name = input("Enter laboratory facility: \n\n")
        self.laboratory_cost = input("Enter laboratory cost: \n\n")

        self.laboratory_list.append(self.laboratory_name)
        self.laboratory_list.append(self.laboratory_cost)

        self.writeListOflaboratoriesToFile(self.laboratory_list)
        

    def displayLaboratories(self):
        ...
        for i in range(len(self.laboratory_list)):
            if self.laboratory_list[i].endswith('\n') == False:
                print(self.laboratory_list[i]+'\n')
            else:
                print(self.laboratory_list[i])

    def writeListOflaboratoriesToFile(self,new_entry):
        '''Writes the laboratories list to laboratories.txt'''
        ...
        while self.repeat:
            ...
            if int(self.option) == 1:
                ...
                while self.cycle:
                    ...
                    if int(self.option) == 1:
                        self.obj_handle.displayLaboratories()
                    elif int(self.option) == 2:
